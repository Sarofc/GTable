//THIS FILE IS GENERATED BY tabtool, DO NOT EDIT IT!
//GENERATE TIME [2020/4/25 5:11:27]

using System.Collections;
using System.Collections.Generic;
using System.Data;

namespace tabtool
{

    public class TbsTestItem
    {
        public int id;
        public string name;
        public tbsIdCount test1;
        public List<int> test2;
        public List<tbsKeyValue> test3;
    }

    public class CfgTestTable : TableManager<TbsTestItem, CfgTestTable>
    {
        public override bool Load()
        {
            TableReader tr = new TableReader();
            DataReader dr = new DataReader();
            DataTable dt = tr.ReadFile(MyConfig.WorkDir+"Test.txt");

            foreach(DataRow row in dt.Rows)
            {
                var data = new TbsTestItem();
                  data.id = dr.GetInt(row["id"].ToString());
                  data.name = (row["name"].ToString());
                  data.test1 = dr.GetObject<tbsIdCount>(row["test1"].ToString());
                  data.test2 = dr.GetIntList(row["test2"].ToString());
                  data.test3 = dr.GetObjectList<tbsKeyValue>(row["test3"].ToString());
                  m_Datas[data.id] = data;
            }
            return true;
        }
    }


    public class TbsachiveItem
    {
        public int id;
        public string name;
        public tbsIdCount test1;
        public List<int> test2;
        public List<tbsKeyValue> test3;
    }

    public class CfgachiveTable : TableManager<TbsachiveItem, CfgachiveTable>
    {
        public override bool Load()
        {
            TableReader tr = new TableReader();
            DataReader dr = new DataReader();
            DataTable dt = tr.ReadFile(MyConfig.WorkDir+"achive.txt");

            foreach(DataRow row in dt.Rows)
            {
                var data = new TbsachiveItem();
                  data.id = dr.GetInt(row["id"].ToString());
                  data.name = (row["name"].ToString());
                  data.test1 = dr.GetObject<tbsIdCount>(row["test1"].ToString());
                  data.test2 = dr.GetIntList(row["test2"].ToString());
                  data.test3 = dr.GetObjectList<tbsKeyValue>(row["test3"].ToString());
                  m_Datas[data.id] = data;
            }
            return true;
        }
    }


    public class TbsZNItem
    {
        public int id;
        public string text;
    }

    public class CfgZNTable : TableManager<TbsZNItem, CfgZNTable>
    {
        public override bool Load()
        {
            TableReader tr = new TableReader();
            DataReader dr = new DataReader();
            DataTable dt = tr.ReadFile(MyConfig.WorkDir+"ZN.txt");

            foreach(DataRow row in dt.Rows)
            {
                var data = new TbsZNItem();
                  data.id = dr.GetInt(row["id"].ToString());
                  data.text = (row["text"].ToString());
                  m_Datas[data.id] = data;
            }
            return true;
        }
    }


    public class TbsENItem
    {
        public int id;
        public string origin;
        public string text;
    }

    public class CfgENTable : TableManager<TbsENItem, CfgENTable>
    {
        public override bool Load()
        {
            TableReader tr = new TableReader();
            DataReader dr = new DataReader();
            DataTable dt = tr.ReadFile(MyConfig.WorkDir+"EN.txt");

            foreach(DataRow row in dt.Rows)
            {
                var data = new TbsENItem();
                  data.id = dr.GetInt(row["id"].ToString());
                  data.origin = (row["origin"].ToString());
                  data.text = (row["text"].ToString());
                  m_Datas[data.id] = data;
            }
            return true;
        }
    }

    public class TableConfig : SingletonTable<TableConfig>
    {
        public bool LoadTableConfig()
        {
            if (!CfgTestTable.Instance.Load()) return false;
            if (!CfgachiveTable.Instance.Load()) return false;
            if (!CfgZNTable.Instance.Load()) return false;
            if (!CfgENTable.Instance.Load()) return false;
            return true;
        }
    }
}
